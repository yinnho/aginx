...
└── agc/              # 命令行客户端 (Rust) - curl for agent://
```
...
**Protected vs Private**：Protected 面向团队协作，未认证连接只能调用 `listAgents`、`ping`、`initialize`、`bindDevice`，需要 JWT token 才能调用 `prompt` 等功能方法。Private 面向个人使用，未认证连接只能 `initialize` 和 `bindDevice`，其他所有方法都需要认证。两种模式都支持绑定和授权两种认证方式。
...
**Website**: [aginx.net](https://aginx.net)
